...
echo "Deploying sysman-diagnosis-agent using agents-cli with APP_URL=${URL_DIAGNOSIS}..."
agents-cli deploy \
  --project="${PROJECT_ID}" \
  --region="${REGION}" \
  ...
  --min-instances=1 \
  --no-confirm-project \
  --update-env-vars="GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=${REGION},GOOGLE_GENAI_USE_VERTEXAI=True,MCP_SERVER_URL=${URL_MCP},FAST_MODEL=${FAST_MODEL},PRO_MODEL=${PRO_MODEL},APP_URL=${URL_DIAGNOSIS}"
