    ...
    async def stream_query(request: Request) -> responses.StreamingResponse:
        ...
        async def generator():
            async for event in method(**(body.get("input") or {})):
                # Sanitize event parts to remove empty {} dictionaries that break Gemini Enterprise UI parser
                if isinstance(event, dict) and "content" in event:
                    content = event["content"]
                    if isinstance(content, dict) and "parts" in content and isinstance(content["parts"], list):
                        content["parts"] = [
                            p for p in content["parts"] 
                            if p and isinstance(p, dict) and any(k in p for k in ("text", "function_call", "function_response", "inline_data"))
                        ]
