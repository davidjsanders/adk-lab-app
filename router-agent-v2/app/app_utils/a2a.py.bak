...
logger = logging.getLogger("router-agent.a2a")
...
async def _a2ui_after_event(
    ...
    Raises:
        None.
    """
    logger.info(f"A2UI Interceptor running on event type: {type(a2a_event)}")
    parts = []
    ...
    if isinstance(a2a_event, TaskStatusUpdateEvent) and a2a_event.status and a2a_event.status.message:
        ...
        logger.info("Found parts in TaskStatusUpdateEvent")
    elif isinstance(a2a_event, TaskArtifactUpdateEvent) and a2a_event.artifact:
        parts = a2a_event.artifact.parts
        logger.info("Found parts in TaskArtifactUpdateEvent")

    if not parts:
        logger.info("No parts found in event")
        return a2a_event
    ...
    for part in parts:
        ...
        if isinstance(part_root, TextPart) and part_root.text:
            text = part_root.text
            logger.info(f"Checking text part (starts with): {text[:20]}...")
            if "<a2ui-json>" in text and "</a2ui-json>" in text:
                logger.info("Found <a2ui-json> tags!")
