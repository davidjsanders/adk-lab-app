#!/bin/bash
# Script to stand up the Confluence Specialist Agent local environment.
...
echo "Cleaning up any stale port bindings (8083, 8005)..."
fuser -k 8083/tcp 8005/tcp >/dev/null 2>&1 || true
sleep 2
...
cleanup() {
  ...
  log_tty "Cleaning up local port bindings (8083, 8005)..."
  fuser -k 8083/tcp 8005/tcp >/dev/null 2>&1 || true
...
echo "=============================================="
echo " Starting SysMan-v2 Confluence Local Env"
echo "=============================================="

# Ensure logs directory exists
mkdir -p "$WORKSPACE_DIR/logs"

# 1. Start Confluence Emulator
...
PORT=$MCP_PORT \
SYSTEM_EMULATORS='{"confluence-app-01": "http://127.0.0.1:8083"}' \
...
echo "=============================================="
echo " Launching Confluence Specialist Playground..."
echo "=============================================="

# Launch playground for the Confluence role
cd "$WORKSPACE_DIR/sysman-v2/sysman-agent"
export AGENT_ROLE="specialist"
export AGENT_CONFIG_FILE="confluence-prd.json"
