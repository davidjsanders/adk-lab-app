#!/bin/bash
# Script to stand up the Linux Specialist Agent local environment.
...
echo "Cleaning up any stale port bindings (8081, 8005)..."
fuser -k 8081/tcp 8005/tcp >/dev/null 2>&1 || true
sleep 2
...
cleanup() {
  ...
  log_tty "Cleaning up local port bindings (8081, 8005)..."
  fuser -k 8081/tcp 8005/tcp >/dev/null 2>&1 || true
...
echo "=============================================="
echo " Starting SysMan-v2 Linux Local Env"
echo "=============================================="

# Ensure logs directory exists
mkdir -p "$WORKSPACE_DIR/logs"

# 1. Start Linux Emulator
...
PORT=$MCP_PORT \
SYSTEM_EMULATORS='{"linux-server-01": "http://127.0.0.1:8081"}' \
...
echo "=============================================="
echo " Launching Linux Specialist Playground..."
echo "=============================================="

# Launch playground for the Linux role
cd "$WORKSPACE_DIR/sysman-v2/sysman-agent"
export AGENT_ROLE="specialist"
export AGENT_CONFIG_FILE="linux-prd.json"
